...
export const createBooking = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [result]: any = await pool.query(
            `INSERT INTO bookings (${columnNameForGuest}, email, suite_name, check_in, check_out, special_requests, amount, payment_status) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [fullName, email, suite, checkIn, checkOut, requests || '', amount || 0, status || 'pending']
        );
